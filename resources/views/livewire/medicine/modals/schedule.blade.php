        ...
        <div>
            <div class="mt-4 text-center">
                ...
                <h3 class="text-xl font-semibold leading-6 text-gray-800 capitalize dark:text-white" id="modal-title">
                    @if ($this->status == 0)
                    <x-banner-modal-icon :title="'CITA PENDIENTE'" :size="'w-16 h-16'" :icon="'calendar-edit'"
                        :titlesize="'xl'" />
                    {{-- CITA --}}
                    @elseif ($this->status == 1)
                    <x-banner-modal-icon :title="'CITA VALIDADA'" :size="'w-16 h-16'" :icon="'calendar-check'"
                        :titlesize="'xl'" />
                    {{-- CITA VALIDADA --}}
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-md p-3" role="alert">
                        <div class="flex">
                            <div class="flex-shrink-0">
                                <svg class="h-3 w-3 text-blue-600 mt-1" xmlns="http://www.w3.org/2000/svg" width="16"
                ...
                optional($januaryAppointment->medicineDocument)->name_document === 'JANUARY-APPOINTMENT')
                @else
                <div class="grid xl:grid-cols-2 xl:gap-6">
                    ...
                    <div class="mt-1 relative w-full group">
                        <x-input wire:model.lazy="pay_date" id="date_pay" label="FECHA DE PAGO" placeholder="INGRESE..."
                            readonly />
                    </div>
                </div>
                ...
                <div x-data="{ selectedOption: @entangle('selectedOption') }">
                    ...
                    @if ($status == 8)
                    <label class="mt-4 grid xl:grid-cols-6">EXTENSION:
                        <x-badge class="ml-2" flat positive label="CONCLUYÓ APTO" />
                    </label>
                    @elseif($status == 9)
                    <label class="mt-4 grid xl:grid-cols-6">EXTENSION:
                        <x-badge class="ml-2" flat negative label="CONCLUYÓ NO APTO" />
                    </label>
