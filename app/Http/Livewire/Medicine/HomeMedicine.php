...
class HomeMedicine extends Component
{
    ...
    public function test()
    {
        ...
        if ($printQuery->type_exam_id == 1) {
            $pdf = PDF::loadView('livewire.medicine.documents.appointment-pdf', compact('printQuery', 'sumappointment'));
            return $pdf->download($printQuery->paymentDate . '-' . $printQuery->appointmentTypeExam->name . ' cita.pdf');
        } else if ($printQuery->type_exam_id == 2) {
            $pdf = PDF::loadView('livewire.medicine.documents.appointment-pdf2', compact('printQuery', 'sumappointment'));
            return $pdf->download($printQuery->paymentDate . '-' . $printQuery->appointmentTypeExam->name . ' cita.pdf');
        }
    }
